...
- 面试问答、常见误区用 `<details>` 折叠收纳；**`<details>` 只装文本与图（` ```mermaid ` / ` ```gantt `），代码块一律放正文**——要展示代码就开正文节（如 `## 落地：Spring 写法`），别塞进折叠块（已断言进 `vinea_check`）。callout 分工：`[!note]` 边界澄清、`[!tip]` 关联实用、`[!warning]` 易错点，紧贴它注解的内容放置，不甩到节末。
...
- 子墙分组范式（样板：`java-锁` → `类加载` → `线程池`）：组序按**阅读因果序**（先主体/流程 → 再规则 → 最后例外），**不按考点热度排**——用户已裁决「类加载」里 `加载器与流程` 必须排在 `委派规则` 之前（先认谁在加载、加载分几步，才谈得上按什么规则委派）。组名 2-5 字写语义不写层级；chip 只写该篇最硬的两个辨识点（术语 · 边界），不写「1 篇/更新于」这类噪音。改了组序要同步 MOC 正文双链序与子笔记 `order` 字段，保持全库只有一条阅读序。
