...
- 笔记两类由 frontmatter `type` 决定（缺省 note）：**MOC**（`type: moc`）正文 = 一句定位 + 子笔记 `[[双链]]` bullet 列表，双链只准写 bullet（散文段落放双链会让 verify-spans 的 `.ventry` 数对不上 `links` 数）；**原子笔记** = 一个概念一篇：正文首块 = **开篇定义句**（`# 标题` 下、首个 H2 之前，一句回答「这个主题是什么」——本体归类 + 定位/边界，≤2 句；不写机制流、不报菜名、不搬数字），随后是 chain 速查块（每行 `标题 | 副标题 | 角标`，4-5 节点封顶；微型/线性笔记可省）与导语（机制主线）；小节按因果推进（机制 → 推论 → 边界 → 复盘）。**定义句讲过的「是什么」正文不再重讲**：首个内容节直接从机制/为什么切入，原有的定义段删掉或压成一句指针。MOC / leetcode 题卡 / `clip` 速记不套这条（各自形态见 note-checkup），机器断言在 `vinea_check.mjs`（缺定义句 = P1）。
