...
2. **骨架**：frontmatter 三件套 → **`:::lede` 块（是什么）** → chain 速查 → 导语（机制主线）→ 小节按因果排（机制 → 推论 → 边界 → 复盘），格式照下两节，别发明格式。
...
<叙事主线句>                              # 导语必在：写机制主线 / 阅读理由 / 一个钩子，禁止逐节复读 chain —— 链是目录，导语是主线，职责不同；lede 已讲过的「是什么」此处不再重复
...
3. **lede 块 + 导语句永远在**：`:::lede` 块在 H1 之后、首个 H2 之前（块内首行写「X 是什么」，可选副行 `**标签：**` 渲染成 chip），导语在 chain 块之后、首个正文节之前（写机制主线）。chain/branch 块推荐但非强制：多节笔记必用；微型/线性笔记（内容即步骤）可用纯文本导语代替。**lede 讲过的「是什么」，正文首节与导语都不许再讲一遍**（首个内容节直接从机制/为什么切入）。
