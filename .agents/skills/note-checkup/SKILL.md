...
「问 AI 速记」：把粘来的重点压成极简卡片，不写 chain、不写完整因果小节。属 `note`（不新开 type），靠收件箱 MOC `随问随记` + 每篇双链回学科墙管理，避免孤儿、也不污染面试八股墙。
...
- 至少一条出链 `[[学科概念]]`：既避出链稀疏 WARN，又把速记并回知识网。
- 落库：挂进 `随问随记` MOC（收件箱）+ 对应学科 MOC 双链；跑 `node .build/verify.mjs && node .build/vinea_check.mjs` 全绿。
