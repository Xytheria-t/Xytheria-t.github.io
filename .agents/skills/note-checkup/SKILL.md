...
<开篇定义句>                              # 必在：一句回答「这个主题是什么」——本体归类 + 定位/边界，≤2 句；不写机制流、不搬数字
...
<叙事主线句>                              # 导语必在：写机制主线 / 阅读理由 / 一个钩子，禁止逐节复读 chain —— 链是目录，导语是主线，职责不同；定义句已讲过的「是什么」此处不再重复
...
3. **定义句 + 导语句永远在**：定义句在 H1 之后、首个 H2 之前（写「X 是什么」），导语在 chain 块之后、首个正文节之前（写机制主线）。chain/branch 块推荐但非强制：多节笔记必用；微型/线性笔记（内容即步骤）可用纯文本导语代替。**定义句讲过的「是什么」，正文首节与导语都不许再讲一遍**（首个内容节直接从机制/为什么切入）。
...
13. **开篇定义句 = 本体定义，不是机制流也不是章节串**：模板「X 是<哪一类东西>，<解决什么 / 与相邻概念的边界>」——术语型写「是一种…机制」，组件型写「是…的组件，负责…（不负责…）」。写成「本文讲 A→B→C」或把正文机制压缩一遍都判违规（机制归导语与正文）。
