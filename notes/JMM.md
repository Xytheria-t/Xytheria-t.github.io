...
写线程在 volatile 写**之前**的所有普通写,都跟着这条 volatile 写一起对读线程可见——「借一个 volatile 变量发布一组成果」就靠它(传递性,8 条里最实用)。
...
- as-if-serial 是底线:单线程内不管怎么重排,**执行结果必须与顺序执行一致**;有数据依赖的语句(`a=1; b=a;`)不会被重排。多线程没有这个兜底,得自己用 hb 关系建立。
...
A：JSR-133 定义的规范,规定线程如何与主内存交互、一个线程的写何时对另一个线程可见,给 Java 一套跨平台一致的并发语义。

Q：happens-before 是时间上的先后吗？

A：不是。A hb B 表示 A 的结果对 B 可见且 A 不会被重排到 B 之后;时间上先发生但无 hb 关系,读到的仍可能是旧值。

Q：volatile 写之前的普通变量,读线程为什么也能看到？

A：程序次序让普通写 hb volatile 写,volatile 变量规则让 volatile 写 hb 读线程的 volatile 读,再由传递性传到读线程的后续操作。
