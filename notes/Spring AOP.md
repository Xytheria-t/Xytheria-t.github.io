...
```chain
横切关注点 | 事务/日志为什么该从业务方法里抽走 | 动机
概念五术语 | JoinPoint/Pointcut/Advice/Aspect/Weaving 对号入座 | 术语
五种通知 | Around 独控全程，其余四个只能观察 | API
切点表达式 | execution 逐段拆 + 指示器组合 | 语法
执行顺序 | 5.2.7 起同切面固定序，跨切面看 @Order | 顺序
织入原理 | AOP 就是动态代理的应用层封装 | 原理
失效边界 | 增强挂在代理上，没走代理就没增强 | 陷阱
```
...
抽离后 `transfer` 看似丢了事务，但调用方拿到的其实是**代理对象**——事务逻辑在代理层被织回。怎么织回，机制见 [[动态代理]]。
