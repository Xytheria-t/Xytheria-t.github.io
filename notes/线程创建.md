...
:::lede
线程创建指在 Java 里把一段任务体交给新执行流运行的方式：本质都落在 `new Thread(...)` 上，区别只在任务体怎么表达（有无返回值、能否抛受检异常）以及是否交给线程池托管。
**推荐：** 交给线程池而非裸建线程 · 显式指定线程名
:::
...
- 上表四种都是**平台线程**（每任务占一个 OS 线程 + 约 1MB 栈）；JDK 21 起的虚拟线程按「任务」记账、海量 IO 场景可扛几十万任务，是另一条路线：[[虚拟线程]]。
- 平台线程真正生出来永远只有 `new Thread(...)` 一条路，其余方式改的都是**任务体怎么表达**，不是线程怎么生出来——线程池见 [[线程池创建]]。
- 「线程」是 [[进程与线程]] 里那个 CPU 调度的最小单位：同进程内共享堆、各自私有栈；先分清进程与线程，才不会混淆「任务体」与「执行体」。
- 生产选型：不需要返回值用 `Runnable`，需要返回值或抛受检异常用 `Callable`；批量任务一律交给池，别 `new Thread` 裸跑。
...
> [!tip] 为什么要命名
> 默认 `Thread-0` 在几十个线程的 jstack 里等于没信息；命名后线程 dump 与日志能直接定位到业务模块，池化场景靠 `ThreadFactory` 统一命名，见 [[线程池创建]]。
...
A：本质上只有 `new Thread()` 一种；常说的三种 / 四种（继承 Thread、Runnable、Callable + FutureTask、线程池）区别只在任务体的表达与是否有返回值。

Q：start() 和 run() 的区别？

A：`start()` 向 JVM 申请新线程、由新线程回调 `run()`；直接调 `run()` 只是当前线程顺序执行一次普通方法。且一个线程实例只能 `start()` 一次，重复调抛 `IllegalThreadStateException`。

Q：Runnable 和 Callable 有什么区别？

A：Runnable 的 `run()` 无返回值、不能抛受检异常（JDK 1.0）；Callable 的 `call()` 有返回值、可抛受检异常（JDK 1.5），需配合 FutureTask 或线程池 `submit`。

Q：为什么不建议直接 new Thread？

A：频繁创建销毁开销大、无上限、无法复用、故障难定位；交给线程池统一管理，参数可控、可监控，见 [[线程池创建]]。
...
- 误区：调了 `run()` 就算并发。不开新线程，代码再像线程也是串行。
